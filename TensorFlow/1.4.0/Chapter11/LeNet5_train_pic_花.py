...
import LeNet5_infernece_pic_hua
import os
import numpy as np
from keras.utils import np_utils

INPUT_DATA = '../../datasets/flower20191118_150_150.npy'
...
#    x = tf.placeholder(tf.float32, [BATCH_SIZE, LeNet5_infernece_pic.IMAGE_SIZE, LeNet5_infernece_pic.IMAGE_SIZE, LeNet5_infernece_pic.NUM_CHANNELS], name='x-input')
    ...
    x = tf.placeholder(tf.float32, [BATCH_SIZE, LeNet5_infernece_pic_hua.IMAGE_SIZE, LeNet5_infernece_pic_hua.IMAGE_SIZE, LeNet5_infernece_pic_hua.NUM_CHANNELS], name='x-input')    
...
#    labels = tf.placeholder(tf.int64, [None], name='labels')    #上面把数字换成了LeNet5_infernece_pic.IMAGE_SIZE等
    ...
    y_ = tf.placeholder(tf.float32, [None, LeNet5_infernece_pic_hua.OUTPUT_NODE], name='y-input')    #上面把数字换成了LeNet5_infernece_pic.IMAGE_SIZE等
    
#    y_ = tf.placeholder(tf.int64, [None], name='y-input')    #上面把数字换成了LeNet5_infernece_pic.IMAGE_SIZE等
    
    regularizer = tf.contrib.layers.l2_regularizer(REGULARIZATION_RATE)
#    y = LeNet5_infernece.inference(x,False,regularizer)
    y = LeNet5_infernece_pic_hua.inference(x,False,regularizer) #x改为images, y改为output
