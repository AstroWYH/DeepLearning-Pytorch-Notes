...
#    with tf.name_scope("layer2-pool1"):
    ...
    with tf.variable_scope("layer6-pool3"):
        pool3 = tf.nn.max_pool(relu3, ksize=[1, 1, 1, 1], strides=[1, 1, 1, 1], padding='SAME')
    ...
    with tf.variable_scope("layer8-pool4"):
        pool4 = tf.nn.max_pool(relu4, ksize = [1,1,1,1],strides=[1,1,1,1],padding="SAME")

    with tf.variable_scope("layer9-conv5"):
        conv5_weights = tf.get_variable(
            "weight", [CONV5_SIZE, CONV5_SIZE, CONV4_DEEP, CONV5_DEEP],
            initializer=tf.truncated_normal_initializer(stddev=0.1))
        conv5_biases = tf.get_variable("bias", [CONV3_DEEP], initializer=tf.constant_initializer(0.0))
...
#        pool3 = tf.nn.max_pool(relu3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
        pool5 = tf.nn.max_pool(relu5, ksize=[1, 1, 1, 1], strides=[1, 1, 1, 1], padding='SAME')
